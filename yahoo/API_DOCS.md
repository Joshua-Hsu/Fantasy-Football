# Yahoo Fantasy Sports API — reference documentation

Source: https://sports.yahoo.com/developer/docs/ (Yahoo Sports Developer
Portal), captured 2026-07-16. Content © Yahoo — kept here as a working
reference for the `pull-yahoo` integration (`src/fantasy_football/yahoo.py`).

Notes for this repo:
- Endpoints: `https://fantasysports.yahooapis.com/fantasy/v2` (unchanged from
  the legacy API; our puller targets these).
- Auth: OAuth 2.0 authorization-code + refresh tokens. NOTE: the "Register
  Your Application / New API Key flow on YDN" section below describes the OLD
  self-serve flow, which no longer offers Fantasy Sports scopes — access now
  goes through the application-submission form on the new portal.
- `draft_analysis` (player sub-resource) is the source for our weekly value
  snapshots.
